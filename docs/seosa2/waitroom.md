...
Resend에는 `waitroom/{sha256(email|product_id|mall) 앞 32자}/{KST 날짜}/{시도 번호}` 키를 보낸다 (이메일 원문은 싣지 않는다).

Resend 공식 문서(2026-09-25 확인): `Idempotency-Key`는 **24시간** 보존되고, 같은 키·같은 본문 재요청은
다시 보내지 않고 원래 응답을 준다. 같은 키·**다른 본문**은 409 `invalid_idempotent_request`, 동시 요청은
409 `concurrent_idempotent_requests`. 거절된 요청의 키를 저장하는지는 문서에 없다.
그래서 시도 번호를 키에 넣는다 — 명확한 거절(4xx) 뒤 같은 날 가격이 바뀐 재시도가 같은 키였다면 본문이 달라
409(«수락 여부 불명»)로 영영 막혔다. 재시도는 명확한 거절 뒤에만 일어나므로(불명은 재시도하지 않는다) 새 키가 안전하다.
항목을 지우고 다시 담아도 계열·날짜·시도가 같으면 같은 키라 공급자가 한 통으로 합친다.
...
**남는 한계 (정확히 한 번은 보장할 수 없다):**
- 수락 뒤 응답 유실: 잡은 다시 보내지 않는다(발송 기록 `claimed`). 운영자가 **24시간 안에 같은 키·같은 본문**으로
  수동 재전송하면 Resend가 합친다. 24시간이 지나거나 본문이 달라지면 공급자 쪽 중복 방지는 없다 —
  수동 재전송 전에 Resend 대시보드의 발송 로그를 먼저 확인할 것.
- 수락 여부를 끝내 확인하지 못한 알림은 사용자에게 안 갔을 수 있다(누락). 자동으로 복구하지 않는다.

검증 (실제 이메일 0건): `scripts/test-v2-waitroom.js`(가짜 DB · Resend 규칙 mock) — 수락 뒤 응답 유실 → 재실행·수동 재전송·다음 날에도
편지함 한 통, 명확한 거절 뒤 가격이 바뀐 재시도 한 통, 동시 실행 두 개 한 통. `scripts/test-v2-waitroom-pg.js`(실제 PostgreSQL 17) —
재적용 · RLS/권한 · 동시 12연결 선점 하나 · 삭제 뒤 이력 보존 · 옵션 · 첫 판 → UPGRADE(중복 흔적이 있으면 실패하고 그대로) · ROLLBACK.
