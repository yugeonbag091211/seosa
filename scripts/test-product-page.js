...
(async () => {
  ...
  section('2. __route=page (HTML)');
  {
    ...
    /*
     * ── 「비슷한 가격의 다른 선택」 (2026-09-07 통합) ─────────────────
     *
     * 예전 이 자리의 제목은 「'{검색어}'의 다른 상품」이었고, 같은 검색어면
     * 무엇이든 실었다. 제목이 «비슷한 가격» 으로 바뀌었으므로 값도 그래야
     * 한다 — 아니면 화면이 거짓말을 한다.
     *
     * 이제 api/_radar.alternativesFor 가 가격대(0.5~1.8배)와 부속을 거른다.
     * 기준 1001 은 29,900원이므로 14,950~53,820원만 남는다.
     *   1003 브리츠 19,900원   → 남는다
     *   1002 소니 289,000원    → 10배 차이라 «비슷한 가격» 이 아니다
     */
    ok(/href="\/p\/1003"/.test(r.text), '비슷한 가격대의 다른 상품은 남는다');
    ok(!/href="\/p\/1002"/.test(r.text), '★ 10배 비싼 상품을 «비슷한 가격» 이라 하지 않는다');
    ok(!/href="\/p\/1005"/.test(r.text), '링크 없는 상품은 내부 링크에서 뺀다');
    ok(/max-age=0, s-maxage=3600/.test(r.headers['cache-control'] || ''), 'Edge 캐시 1시간');
    /*
     * ★ 문구가 바뀌었다 (2026-09-04 감사).
     *
     *   예전 단언은 "배송비·쿠폰" 이라는 낱말이 본문에 있는지만 봤다. 그런데
     *   그 문구("배송비·쿠폰·카드 할인은 포함되지 않았어요")는 확인할 수 없는
     *   주장이었다 — 쿠팡 파트너스 검색 API 응답에는 가격 종류를 말해 주는
     *   필드가 없고(productPrice 하나뿐), 실측으로 그 값이 상품 페이지 가격과
     *   달랐다 (7912306911: API 22,320원 ↔ 페이지 26,900원 / 와우 23,610원).
     *
     *   그래서 단언을 뒤집는다: 이제는 "모른다는 사실을 밝히는가" 를 본다.
     *   옛 문구가 되살아나면 아래 두 번째 단언이 깨진다.
     */
    ok(/알려주지 않아서/.test(r.text) && /실제 결제 금액은 판매처에서 확인/.test(r.text),
      '가격 종류를 모른다는 사실과 결제 금액 확인 안내가 있다');
    ok(r.text.indexOf('쿠폰·카드 할인은 포함되지 않았어요') < 0,
      '★ 확인되지 않은 "쿠폰·카드 할인 미포함" 단정이 되살아나지 않았다');
  }
  {
