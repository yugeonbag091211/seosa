...
/*
 * Test 1b — noMatch 를 «줄이려고» 이 게이트를 풀면 안 된다 (2026-09-20 감사).
 *
 * ── 왜 이 시험을 따로 두는가 ──────────────────────────────────────
 *
 * 수집률이 낮아 보이면 «매칭을 느슨하게 하자» 는 압력이 늘 생긴다. 그런데
 * 2026-09-20 실측이 그 유혹의 값을 정확히 매겨 두었다.
 *
 *   그날 쿠팡 noMatch 596개를, 그날 실제로 받은 응답 2,162건 전부와 대조:
 *     · 응답 어딘가에 우리 product_id 가 있었다        290개 (48.7%)
 *       └ 그중 vendorItemId 가 «우리 옵션» 인 것          8개 ( 2.8%)
 *       └ 나머지는 «다른 옵션» 이다                     282개 (97.2%)
 *     · 응답에 아예 없었다 (검색어로 도달 불가)         306개 (51.3%)
 *
 * 즉 "응답에 있는데 못 잡은 것" 처럼 보이는 290개 중 97.2% 는 다른 옵션이다.
 * product_id 만 보고 채택하면 수집률은 596 → 306 으로 «좋아 보이지만»,
 * 282개 상품에 남의 옵션 가격이 오늘 가격으로 박힌다.
 *
 * 그래서 이 게이트를 푸는 방향의 변경은 개선이 아니라 회귀다.
 */
{
  const t = target('P1b', 'I1b', 'OURS');
  // 같은 productId 의 다른 옵션들만 돌아온 상황 — 운영에서 가장 흔한 모양이다.
  const r = pickOption(t, [
    item('P1b', 'I1b', 'OTHER1', 12000),
    item('P1b', 'I1b', 'OTHER2', 9900),
    item('P1b', 'I1b', 'OTHER3', 31000)
  ]);
  eq('[T1b] product_id 가 맞아도 우리 옵션이 없으면 채택하지 않는다', r.item, null);
  eq('[T1b] 사유는 OPTION_MISMATCH', r.reason, 'OPTION_MISMATCH');
  check('[T1b] ★ 가장 싼 남의 옵션을 «그래도» 고르지 않는다',
    !r.item, r.item ? JSON.stringify(r.item) : '채택 없음');
}
