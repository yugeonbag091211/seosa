...
const CLASSIFY_SYSTEM = [
  ...
  'B  세상에 존재하는(또는 존재한다고 전제된) 대상에 대한 설명·사실·감상 요청.',
  '   작품·인물·사건·개념 등이 대상이다. 사려는 의도가 없다.',
  '   그 대상이 상점에서 팔릴 수 있는 물건인지는 판단과 무관하다.',
  ...
  '',
  '출력 형식: 대문자 한 글자(A B C D E 중 하나)만. 다른 말은 절대 붙이지 마라.'
].join('\n');

/**
 * @returns {{intent:string, followUp:boolean}|null}  실패하면 null
 */
async function classifyIntent(q, hist) {
  const msgs = [{ role: 'system', content: CLASSIFY_SYSTEM }];
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({ model: CLASSIFY_MODEL, messages: msgs, max_tokens: 8, temperature: 0 })
    });
    if (!r.ok) throw new Error(`${r.status}`);
    const data = await r.json();
    const out = String((((data.choices || [])[0] || {}).message || {}).content || '').trim().toUpperCase();
    const m = out.match(/\b([A-E])\b/);
    if (!m) throw new Error(`형식 불명: ${out.slice(0, 20)}`);
    return { intent: m[1] };
  } catch (e) {
...
P.roleShop = [
  ...
  '- 조건이 어느 정도 모였으면 SEOSA 검색으로 이어준다. 어떤 말로 검색하면 되는지',
  '  알려주고, 검색하면 오늘 가격과 가격 기록을 함께 볼 수 있다고 안내한다.',
...
P.dataRules = [
  ...
  '  SEOSA에서 그 이름으로 검색하면 실제 가격을 볼 수 있다고 안내한다.',
  '',
  '[가격 기록이 없는 상품]',
...
const SYSTEM_BASE = [
  ...
  '  B 일반 지식·문화  세상에 존재하는 대상(작품·인물·사건·개념)에 대한 설명·사실·감상 요청.',
  '                  사려는 의도가 없다. 그 대상이 팔리는 물건인지는 무관하다.',
  '  C 쇼핑 추천      무엇을 살지 고르는 데 도움을 청한다. 아직 특정 제품을 지목하지 않았다.',
  ...
  '    조건이 하나뿐이면 가장 중요한 것 하나를 묻고, 여러 개가 이미 나왔으면',
  '    그것들을 그대로 확인한 뒤 남은 것 하나만 묻거나 바로 검색으로 안내한다.',
  '  조건이 어느 정도 모였으면 SEOSA 검색으로 이어준다. 어떤 말로 검색하면 되는지',
  '  알려주고, 검색하면 오늘 가격과 가격 기록을 함께 볼 수 있다고 안내한다.',
  '  같은 되묻기를 두 번 반복하지 마라.',
