...
/*
 * ★ 숫자 한가운데서 시작하지 않는다 — (?<![\d.]) (2026-09-24, SEOSA 2.0 조사관 작업 중 발견).
 *
 *   예전 규칙은 소수점 한 자리까지만 받아서, 두 자리 소수가 오면 정규식이 뒤로 물러나
 *   숫자의 «뒷부분» 에서 다시 시작했다. 판매자 제목에서 그대로 틀린 사양이 나왔다.
 *     "…1.19kg"   → 19kg (19,000g)     "1.75L 전기포트" → 5L
 *     "12.34cm"   → 34cm
 *   AI 답변·비교표가 이 값을 근거로 "가장 가볍다" 를 말하므로 틀린 사양은 틀린 추천이 된다.
 *   이제 소수 2~3자리까지 받고, 숫자·소수점 바로 뒤에서는 시작하지 않는다.
 */
const UNITS = [
  ['size_inch',    /(?<![\d.])(\d{1,2}(?:\.\d{1,2})?)\s*인치/,       v => toNum(v)],
  ['size_inch',    /(?<![\d.])(\d{1,2}(?:\.\d{1,2})?)\s*"(?!\w)/,    v => toNum(v)],
  ['capacity_ml',  /(\d{2,5})\s*ml\b/i,                            v => toNum(v)],
  ['capacity_ml',  /(?<![\d.])(\d{1,2}(?:\.\d{1,2})?)\s*L(?![a-zA-Z])/, v => Math.round(toNum(v) * 1000)],
  ['weight_g',     /(?<![\d.])(\d{1,4}(?:\.\d{1,3})?)\s*kg\b/i,      v => Math.round(toNum(v) * 1000)],
  ...
  ['length_cm',    /(?<![\d.])(\d{1,4}(?:\.\d{1,2})?)\s*cm\b/i,      v => toNum(v)]
];
 ...
 */
/*
 * ★ "그램" 의 "램" 을 메모리로 읽지 않는다 — (?<![가-힣])램 (2026-09-24).
 *   "LG 그램 17 17Z90S 램 32GB" 가 램 17GB 로 읽혔다(17 은 화면 인치). LG 그램은
 *   카탈로그의 주력 노트북이라 비교표의 램이 통째로 틀렸다. 한글 음절 바로 뒤의 "램" 은
 *   단어의 일부다 — "메모리 램 16GB" 처럼 띄어 쓴 경우와 "램16GB" 는 그대로 잡힌다.
 */
const RAM_RE = [
  /(?<![가-힣])램\s*(\d{1,3})\s*(?:gb|기가)?/i,
