...
async function fetchCoupang(keyword, limit = 6, opts = {}) {
  ...
  const items = r.items.map(it => ({
    ...
    /*
     * 배송 정보 (2026-08-30 3차 감사에서 복구).
     *
     * ★ 이 두 줄이 없어서 배송 기능 전체가 죽어 있었다.
     *
     *   api/_coupang.js 는 쿠팡 응답에서 isRocket / isFreeShipping 을 꺼내
     *   주석까지 달아 두고 있었다("그대로 흘려보내기만 한다"). 그런데 바로
     *   위 계층인 이 함수가 항목을 **새 객체로 다시 만들면서** 필드 목록에
     *   둘을 빠뜨렸다. 그래서 쿠팡에서 온 값이 여기서 통째로 버려졌다.
     *
     *   결과:
     *     · 검색 결과 20건에 배송 데이터 0건 (2026-08-30 실측)
     *     · 프론트의 배송 필터는 값이 있는 결과에서만 뜨도록 만들어져 있어서
     *       **한 번도 뜬 적이 없다**
     *     · scripts/test-filters.js 가 이 필터 로직을 테스트하지만 그 파일은
     *       npm test 체인 밖이라 아무도 눈치채지 못했다
     *
     *   ★ 저장하지는 않는다. products 에 컬럼이 없고, 마이그레이션 없이
     *     응답에만 실어 보낸다 — 추가 필드라 API 계약을 깨지 않는다.
     *     따라서 이 값은 "방금 검색한 결과"에만 있고 홈 섹션(DB 조회)에는
     *     없다. 프론트는 값이 있을 때만 배송을 그린다.
     */
