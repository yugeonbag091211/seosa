...
POST /api/ai   (토큰 필수 — 익명 호출로 요금이 나가지 않는다)
   ...
   → 의도 분류  (anthropic/claude-haiku-4.5, 8초, 직전 4턴)
   → 상품 질문이면 검색·시세 맥락을 붙여 본 모델 호출
   → 실패하면 예약한 쿼터를 되돌린다                                    ★
```
...
| **높음** | **OpenRouter 크레딧 402** | AI Concierge 가 실제로 답을 못 한다. 코드 문제가 아니라 잔액 문제. 오류 처리는 정상 동작 확인. |
