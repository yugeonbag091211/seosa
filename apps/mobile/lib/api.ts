...
export function createApiClient(fetcher: typeof fetch = fetch, timeoutMs = 12000) {
  async function get(path: string, params: Record<string, string>, signal?: AbortSignal): Promise<unknown> {
  ...
  return {
    async search(keyword: string, signal?: AbortSignal): Promise<Product[]> {
      const data = await get('/api/search', { keyword: keyword.trim() }, signal);
    ...
    async product(id: string, mall?: string, signal?: AbortSignal): Promise<ProductDetail> {
      const data = await get('/api/history', { __route: 'product', pid: id, ...(mall ? { mall } : {}) }, signal) as Partial<ProductDetail>;
    ...
    async history(product: Product, signal?: AbortSignal): Promise<PriceHistory> {
      ...
      const data = await get('/api/history', { productId: product.productId, mall: product.mall, vendorItemId: product.vendorItemId || '', deal: '1' }, signal) as Partial<PriceHistory>;
